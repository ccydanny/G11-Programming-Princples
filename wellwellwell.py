...
def update_timetable(studentid):
    ...
    print(f'[2] - [Credit Hours] {selected_entry[3]}')
    print(f'[3] - [Week] {selected_entry[2]}')
...
def delete_timetable_entry(studentid):
    with open(f'timetables_{studentid}.txt', 'r+') as file:
        ...
        for line in lines:
            data = line.strip().split(',')
            print(data[:2])
            course_code_to_delete = input("Enter the course code to delete: ")
            if data[0] != course_code_to_delete:
                file.write(line)
        file.truncate()
...
def update_assignment(studentid):
    ...
    with open(f'assignments_{studentid}.txt', 'r+') as file:
        ...
        for line in lines:
            data = line.strip().split(',')
            print(data[:4])
            course_code_to_update = input("Enter course code: ")
            if data[0] == course_code_to_update:
                print(f"Course Code: {data[0]}")
                print(f"Course Name: {data[1]}")
                ...
                print("Course code was not found in the timetable")
        file.truncate()
    ...
    while confirmation.lower() != "y":
        ...
        if userInput == "1": # Mark Attendance
            ...
            if userInputdisplay == "1":
                ...
                display_multiple_assignment(studentid)
            else:
                print("---Back to Homepage---")
    ...
    with open(attendance_file, 'r+') as file:
        ...
        for line in lines:
            ...
            if data[0] == subject_code and data[2] == week:
                data[5] = "ABSENT"  # Special case, where the student checked in under illegal circumstance. Won't count to attendance %.
                line = ','.join(data) + '\n'
                updated = True
            file.write(line)
